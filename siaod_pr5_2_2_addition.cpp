...
struct BankAccount {
    int accountNumber;    // 7-значный номер счета (ключ для поиска)
    char name[50];        // ФИО клиента
    char address[100];    // Адрес клиента
};
...
void printRecord(const string& filename, int index) {
    if (index == -1) return; //
    
    // Открываем файл и перемещаемся к нужной записи
    ifstream file(filename, ios::binary);
    file.seekg(index * sizeof(BankAccount)); // Вычисляем смещение записи
    
    BankAccount acc;
    file.read(reinterpret_cast<char*>(&acc), sizeof(BankAccount)); // Читаем запись

    cout << "Найдена запись:" << endl;
    cout << "Индекс: " << index << endl;
    cout << "Номер счета: " << acc.accountNumber << endl;
    cout << "ФИО: " << acc.name << endl;
    cout << "Адрес: " << acc.address << endl << endl;
